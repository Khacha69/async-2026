...
def make_coffee(customer_name):
    ...
    sum(i * i for i in range(1000000))  # จำลองงานคำนวณ (CPU-bound) เล็กน้อย ใช้เวลา 5 วินาที
    sleep(5)  # นอนการทำงานของ Thread นี้ไว้ 5 วินาทีเต็ม
...
def main():
    ...
    print(f"{ctime()} | [Main PID: {main_pid}] [Main TID: {main_tid}] === เริ่มระบบชงอัตโนมัติแบบ Synchronous ===")
    start_time = time()
    start_cpu = process_time()  # เริ่มจับเวลา CPU

    # ชงกาแฟตามลำดับคิวทีละคน (ทีละคน)
    ...
    print(f"การใช้หน่วยความจำ Memory (RAM) ที่ใช้: {mem_mb:.2f} MB")
